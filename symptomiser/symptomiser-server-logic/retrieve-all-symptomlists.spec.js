...
const { mongoose, models: { Symptom, SymptomList } } = require('symptomiser-data')

describe('server logic - retrieve all symptomLists', () => {
    ...
    it('should succeed under the right conditions', async () => {
        const symptomLists = await retrieveAllSymptomLists()

        expect(symptomLists.length).to.equal(1)

        const [{symptomList: [retrievedSymptom]}] = symptomLists
